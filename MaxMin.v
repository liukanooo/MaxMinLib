...
Definition option_le: option nat -> option nat -> Prop := 
...
Definition option_plus: option nat -> option nat -> option nat :=
  fun x y => match x, y with
...
Theorem option_le_antisym: forall x y, option_le x y -> option_le y x -> x = y.
Proof. 
...
Theorem option_le_total: forall x y, {option_le x y} + {option_le y x}.
Proof. 
  intros. destruct x, y; simpl in *; auto. 
  apply Nat_le_total.
Qed.

#[export] Instance option_le_TotalOrder: TotalOrder option_le := {
  le_refl := option_le_refl;
  le_trans := option_le_trans;
  le_antisym := option_le_antisym;
  le_total := option_le_total
}.